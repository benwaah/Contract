...
contract Congress is owned
{
	...
	/*change rules*/
	function changeVotingRules(uint minimumQuorumForProposals,
							   uint minutesForDebate,
							   int marginOfVotesForMajority) onlyOwner
	{
				  ...
				  string justificationText) onlyMembers returns (uint voteID)
	{
		...
		p.numberOfVotes++;
		// If they support the proposal then nncrease scorem otherwise decrease it
		if (supportsProposal)
		{
			p.currentResult++;
		}
	...
	function executeProposal(uint proposalNumber, bytes transactionBytecode) returns (int result)
	{
		...
		if (now < p.votingDeadline || p.executed || p.proposalHash != sha3(p.recipient, p.amount, transactionBytecode) || p.numberOfVotes < minimumQuorum)
			throw;
